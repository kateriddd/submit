...
def _as_action_sequence(decoded_action, cfg):
    ...
    k = int(cfg.model.policy.action_stacking)
...
def eval_libero_fast(model, device, cfg, iter_=0, log_dir="./",
    ...
    for task_id in tasks:
        ...
        for init_state_id in range(episodes_to_run):
            ...
            while not (done or truncated or (t > timeLimit)):
                ...
                state_obs = info.get('state_obs')
                if state_obs is None:
                    state_obs = obs if obs.shape == (13,) else np.zeros(13)
                #CODE MODIFIED TO BE CALLABLE WITH IMAGES FROM HOMEWORK 1, adapted to use images or pose data
                if cfg.grpo.wm:     #uses images
                    # Image Preprocessing
                    img_input = cv2.resize(obs, (64, 64)) if obs.shape[:2] != (64, 64) else obs
                    img_input = img_input.copy()
                    img_tensor = torch.from_numpy(img_input).float().permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
                    
                    # 2. Goal & Text tokens
                    g_txt = model.encode_text_goal(instruction).to(device)
                    g_img = model.goal_img.to(device)
                    # 3. Forward Pass (No Pose)
                    out = model.forward(
                        images=img_tensor,    
                        goals_txt=g_txt,      
                        goal_imgs=g_img,      
                ...
                action_logits = out[0]
                decoded = model.decode_action(action_logits).cpu().detach().numpy()

                action_seq = _as_action_sequence(decoded, cfg)
                if t == 0:
                    print(f"action_seq shape: {action_seq.shape}")
                assert action_seq.shape == (cfg.model.policy.action_stacking, cfg.action_dim), (
                    f"Expected action_seq shape {(cfg.model.policy.action_stacking, cfg.action_dim)}, got {action_seq.shape}"
                )
                last_action = action_seq[-1].copy()
                ...
                for act_ in action_seq:
                    ...
                    rewards.append(reward)
                    infos.append(info)
                    if info.get('success', False):      #track the success rate for the ep
    ...
    successes = [traj['success'] for traj in trajectory_data]
    success_rate = np.mean(successes) if successes else 0.0
    if not cfg.testing and wandb is not None:
        wandb.log({"avg reward_fast": avg_reward, "success_rate": success_rate})
        if last_video_path is not None and render:
            ...
            os.remove(last_video_path) # Deletes the local file after WandB has it
    return episode_stats
